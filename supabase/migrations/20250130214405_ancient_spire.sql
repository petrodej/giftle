...
CREATE OR REPLACE FUNCTION notify_new_member_v4()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  PERFORM extensions.http(request);
...
-- Create new trigger
CREATE TRIGGER send_member_invite_v4
  AFTER INSERT ON project_members
  FOR EACH ROW
  WHEN (NEW.status = 'pending' AND NEW.email IS NOT NULL)
  EXECUTE FUNCTION notify_new_member_v4();
