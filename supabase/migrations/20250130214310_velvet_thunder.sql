...
CREATE OR REPLACE FUNCTION notify_new_member_v3()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Send email notification using http extension with the correct function signature
...
CREATE TRIGGER send_member_invite_v3
  AFTER INSERT ON project_members
  FOR EACH ROW
  WHEN (NEW.status = 'pending' AND NEW.email IS NOT NULL)
  EXECUTE FUNCTION notify_new_member_v3();
