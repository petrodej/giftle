...
CREATE OR REPLACE FUNCTION notify_new_member_v6()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  PERFORM extensions.http(
    method := 'POST',
    uri := current_setting('app.settings.notify_url', true),
    headers := ARRAY[
...
-- Create new trigger
CREATE TRIGGER send_member_invite_v6
  AFTER INSERT ON project_members
  FOR EACH ROW
  WHEN (NEW.status = 'pending' AND NEW.email IS NOT NULL)
  EXECUTE FUNCTION notify_new_member_v6();
