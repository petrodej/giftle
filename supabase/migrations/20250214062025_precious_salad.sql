...
DROP POLICY IF EXISTS "Update own suggestions" ON gift_suggestions;
...
CREATE POLICY "Update own suggestions"
  ...
  USING (
    ...
    AND
    -- And must still be an active member or creator
    EXISTS (
      SELECT 1 
      FROM project_members pm
      JOIN gift_projects gp ON gp.id = pm.project_id
...
CREATE POLICY "Delete suggestions"
  ...
  TO authenticated
  USING (
    -- Can delete if you're the suggestion creator
    (suggested_by = auth.uid())
    OR
