...
-- Create composite types for HTTP requests if they don't exist
DO $$
BEGIN
...
CREATE OR REPLACE FUNCTION notify_new_member_v8()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
...
-- Create new trigger
CREATE TRIGGER send_member_invite_v8
  AFTER INSERT ON project_members
  FOR EACH ROW
  WHEN (NEW.status = 'pending' AND NEW.email IS NOT NULL)
  EXECUTE FUNCTION notify_new_member_v8();
